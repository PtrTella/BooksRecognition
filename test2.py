...
class BookIdentifier:
    ...
    e matching contro un database di template.
    """
    ...
    def __init__(
        ...
        """
        Parametri:
            use_sift            : True -> prova SIFT, se non disponibile passa a ORB
            nfeatures_orb       : numero di feature ORB se SIFT non è disponibile
            color_hist_bins     : numero di bin per canali H, S, V
            ratio_test          : soglia Lowe ratio test (0..1), più bassa = più severa
            min_good_matches    : minimo numero di good matches per considerare un template
            min_color_similarity: minimo coeff. di similarità colore per accettare un match
            alpha_local         : peso dei match locali nel punteggio totale
            beta_color          : peso della similarità colore nel punteggio totale
        """
        ...
        # db: book_id -> dict con kp, des, hist, immagine opzionale
    ...
    # --------- utility per descrittori + colore ---------
    ...
    def match_book(
        ...
        """
        Prova a identificare il libro ritagliato nella ROI.
        Ritorna:
            (book_id, punteggio_totale) o (None, 0) se non riconosciuto.
        """
        ...
        for book_id, data in self.templates.items():
            ...
            if num_good < self.min_good_matches:
                continue  # troppo pochi match

            # similarità colore (più alto = più simile)
            color_sim = cv2.compareHist(hist_t, roi_hist, cv2.HISTCMP_CORREL)

            if color_sim < self.min_color_similarity:
                # colore troppo diverso, probabilmente non è lo stesso libro
                continue
...
if __name__ == "__main__":
    ...
    for scene_file in scene_images:
        ...
        for (x1, y1, x2, y2) in boxes:
            ...
            book_id, score = identifier.match_book(roi)
            print(f"Book at [{x1},{y1},{x2},{y2}] -> ID: {book_id}, Score: {score:.2f}")

            if book_id is not None:
                # disegna bbox + label SOLO sui libri riconosciuti
                cv2.rectangle(vis, (x1, y1), (x2, y2), (0, 255, 0), 2)
                label = f"{book_id} ({int(score)})"
                cv2.putText(
                    vis,
                    label,
                    (x1 + 2, y1 + 20),
        ...
        cv2.imshow("Recognized Books", vis)
        cv2.waitKey(0)
        cv2.destroyAllWindows()